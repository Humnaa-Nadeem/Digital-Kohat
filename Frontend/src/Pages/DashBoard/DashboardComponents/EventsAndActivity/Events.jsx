...
import { AddNewEvent, deleteTheEvent, SendEventTabDataToDb, UpdateExtraActivities } from "../../../../ApiCalls/DashBoardApiCalls";
...
export const EventManagingForm = ({ dashboardData, setDashboardData }) => {
    // =========================
    ...
    const handleDelete = (title) => {
        setDashboardData(prev => ({
            ...prev,
            eventData: prev.eventData.filter(ev => ev.title !== title)
        }));
        deleteTheEvent(title);
    };
        ...
        <section className="form-area">
            ...
            <br></br>
            {
                (dashboardData.eventData)
                    ?
                    EventCards(dashboardData, handleDelete)
                    :
                    <></>
            }
            <br></br>
...
const EventCards = ({ eventData }, handleDelete) => {
    if (!eventData || eventData.length === 0) return null; // hide cards if no events
    return (
