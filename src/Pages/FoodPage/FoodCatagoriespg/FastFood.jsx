...
import { FastFoodList, FastFoodCardsData, Food_Details } from "../../../Store/Food_store";
...
export const FastFoodPage = () => {
    useEffect(() => {
        window.scrollTo(0, 0)
    }, []);

    const [allCrds, setAllCrds] = useState(FastFoodCardsData);
    const [Crds, setCrds] = useState(FastFoodCardsData);
    ...
    useEffect(() => {
        const staticList = FastFoodList.map(item => ({ name: item.name, id: item.id }));
        setListData(staticList);

        GetFoodCrdsDtaFrmDB((dbData) => {
            if (!dbData || !Array.isArray(dbData)) return;
            const filteredDb = dbData.filter(item => item.serviceType === "Fast Food");
            const merged = [...FastFoodCardsData, ...filteredDb];
            setAllCrds(merged);
            setCrds(merged);
                ...
                <section className="food-cata-pg-sec">
                    <div className={(showList) ? "food-lft-sec food-showList" : "food-lft-sec"} >
                        ...
                        <div className="institute-hd-lst">
                            <h2 className="food-institute-hd">Fast Food</h2>
                    ...
                    <div className="food-main-sec">
                        ...
                        <div className="food-cata-banner">
                            <h1 className="fd-cata-pg-main-hd">Quick Bites & Fast Flavors</h1>
                        ...
                        <div className="food-card-cont">
                            ...
                            )) : <div className="no-results"><h3>No fast food found matching your criteria.</h3><button onClick={() => { setFilterPrice("all"); setFilterDelivery(false); }}>Reset Filters</button></div>}
                        </div>
                    </div>
                </section>
            }
