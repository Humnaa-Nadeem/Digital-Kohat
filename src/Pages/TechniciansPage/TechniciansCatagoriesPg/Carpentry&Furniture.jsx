...
// IMPORT CARPENTRY & FURNITURE STORE
import { CF_ExpertCrdDta, CF_ExpertDetails, CarpFurnitureExperts } from "../../../Store/Techcn_Store";

export const CarpFurnitureCata = () => {

    // Scroll to top on page load
    useEffect(() => { window.scrollTo(0, 0); }, []);

    // States
    let [showList, setShowlist] = useState(false); //for small screens
    let [userTags, setUsertags] = useState(false); //for small screens
    let [Crdsdta, setCrdsdta] = useState(CF_ExpertCrdDta);
            ...
            <main>
                <section className="Tech-Cata-Pg-Sec">
                    ...
                    <div className="Tech-cata-pg-banner">
                        <h1 className="tech-cata-h1">Find Carpentry & Furniture Experts</h1>
                        <p className="tech-cata-p">
                            Skilled carpenters and furniture craftsmen for your home and office projects.
                        </p>
                        <SearchBar SearchedInst={setCrdsdta} AllInst={CF_ExpertCrdDta} />
                    ...
                    <div className="tech-cata-fltr-crdsCont">
                        ...
                        <div className={showList ? "tech-cata-fltr display" : "tech-cata-fltr"}>
                            ...
                            <ul className="list">
                                {CarpFurnitureExperts.map((v, i) => (
                                    <li key={i} onClick={() => { navigate(`/tech/carp-furn?id=${v.id}`); setShowlist(false); }}>
                                        {v.ExpertName}
                                    </li>
                                ))}
                                <li onClick={() => { navigate("/tech/carp-furn"); setShowlist(false); }}>Back To Cards</li>
                        ...
                        <div className="tech-cata-crdCont">
                            ...
                            {id ? (
                                // -------------------- DETAILS PAGE --------------------
                                CF_ExpertDetails.filter(v => v.id === Number(id)).map((v, i) => (
                                    <div className="resume-box" key={i}>
                                        <div>
                                            <img src={v.img} alt="Profile" className="profile-img" />
                                            <h1>{v.Title}</h1>
                                            <div className="role">Professional Carpentry & Furniture Expert</div>
                                        </div>
                                ...
                                <div className="cata-card-cont">
                                    {Crdsdta.map((v, i) => (
                                        <div className="cata-pg-card" key={i}>
                                            <img src={v.img} alt="Carpentry Expert" />
                                            <div className="cata-pg-card-content">
                                                <h3>{v.InstName}</h3>
                                                <p>{v.Desc}</p>
                                                <button
                                                    className="cata-pg-card-btn"
                                                    onClick={() => navigate(`/tech/carp-furn?id=${v.id}`)}
                                                >
                                                    {v.btn_txt}
                                                </button>
                                            </div>
